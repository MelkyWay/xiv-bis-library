...
- Keeps row notes concise in-table and shows full note details in hover tooltips.
- Supports localization: English, French, German, Japanese, Korean, Simplified Chinese, and Traditional Chinese.
...
- Results table columns: `Job`, `Category` (when filtering on `All`), context column (`Info`/`Tier`/`Encounter`/`Patch`), `Set/Link`, `Damage`, `Source`, and actions.
- `Set/Link` values are clickable and open the row set URL; note tooltips still appear on hover/focus.
...
- `note` example:
  ...
  - `text` is shown in the `Set/Link` column (truncated in-table when long), links to the row set URL, and appears first in the hover tooltip.
- `damage` examples:
...
3. Run `.github/workflows/deploy-pages.yml` from the Actions tab (`workflow_dispatch`).
4. Wait for the deploy job to finish, then hard-refresh `https://melkyway.github.io/xiv-bis-library/`.

Troubleshooting:

- If the browser requests `/src/main.ts` (404), Pages is serving repository source files instead of the built artifact.
- If `favicon-v6.png` loads from `/favicon-v6.png` (404), the same root-cause applies.
- Fix: keep Pages source on `GitHub Actions` and deploy the `dist/` artifact from the workflow.
